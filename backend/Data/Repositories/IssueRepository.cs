    ...
    public async Task<Issue?> CreateIssue(Issue issue)
    {
        ...
        string query =
            @"
                ...
                RETURNING id, user_id, priority, category, name, description, creation_date, solved_date, solved;
            ";

        using var command = new NpgsqlCommand(query, connection);
        command.Parameters.AddWithValue("@UserId", issue.UserId);
        command.Parameters.AddWithValue("@Priority", issue.Priority.ToString().ToLower());
        command.Parameters.AddWithValue("@Category", issue.Category.ToString().ToLower()); // voeg toe automatisch prioriteit
